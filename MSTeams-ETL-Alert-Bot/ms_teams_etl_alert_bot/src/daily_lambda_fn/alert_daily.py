...
def get_records():
    ...
    cursor.execute("SELECT customer, source, lrd, status, info, airflow"
                             " FROM admin.vw_etl_current_status WHERE (len(info) < 1 OR info ISNULL)"
                             " AND source <> 'MTA Build'"
                             " AND status <> 'success';")
...
def send_email(msgs):
    ...
    to_address = "etlfailure@associationanalytics.com"
    subject = "ETL Monitor"
    body = msgs
    from_address = "etlfailure@associationanalytics.com"
...
main()
print('End DAILY etl_monitor_send_teams_message.py')